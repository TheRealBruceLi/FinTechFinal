...
x6 = data6
y_values = prices
# Can increase cycles but it usually just goes to inf or nan
cycles = 15001
with tf.Session() as sess:
    ...
    for i in range(cycles):
        ...
        if i % 1000 == 0:
            ...
            mean_squared_error = squared_error_sum / len(valPrices)
            print("validation loss:",mean_squared_error)
        if i == cycles-1:
            answer = (sess.run([coeff1, coeff2, coeff3, coeff4, coeff5, 
            coeff6, intercept]))
            for i in range(6):
